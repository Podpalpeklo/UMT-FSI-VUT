...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\begin{axis} [xlabel = {Protažení $\lambda_1$},
		ylabel = {Přetvoření},
		grid,
		height = 6cm,
		width = 10cm,
		...
		\addplot[domain=0:4, samples=100, Goldenrod, thick] {1/2*(x*x-1)};
		\addplot[domain=0:4, samples=100, Green, thick] {x-1};
		\addplot[domain=0:4, samples=100, Blue, thick] {ln(x)};
		\addplot[domain=0:4, samples=100, Red, thick] {1/2*(1-1/(x*x)};
